...
export async function cloneCatalogRows(
  ...
  const skus = await ctx.db.query("pos_inventory_skus").withIndex("by_outlet_active", (q) => q.eq("outlet_id", sourceOutletId).eq("active", true)).collect();
  for (const s of skus) {
  ...
  const products = await ctx.db.query("pos_products").withIndex("by_outlet_active_sort", (q) => q.eq("outlet_id", sourceOutletId).eq("active", true)).collect();
  ...
  return { skus: skus.length, products: products.length, components: componentCount };
}
