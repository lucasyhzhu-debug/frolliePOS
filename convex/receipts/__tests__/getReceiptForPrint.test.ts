...
describe("getReceiptForPrint", () => {
  it("defaults the receipt footer to the English 'Thank you!' when no pos_settings row exists", async () => {
    ...
    const { staffId, sessionId, outletId } = await seedStaff(t, "staff");
    const txnId = await seedPaidTxn(t, staffId, outletId, Date.now());
  ...
  it("returns null for an invalid session", async () => {
    const t = convexTest(schema);
    const { staffId, outletId } = await seedStaff(t, "staff");
    const txnId = await seedPaidTxn(t, staffId, outletId, Date.now());
    const fake = await t.run(async (ctx: any) => {
  ...
  it("returns null for a staff member reading a txn outside server-today", async () => {
    ...
    const txnId = await seedPaidTxn(t, staffId, outletId, twoDaysAgo);
    const res = await t.query(api.receipts.public.getReceiptForPrint, { sessionId, txnId });
    expect(res).toBeNull();
  });
...
describe("getReceiptForPrint — payment method label", () => {
  ...
  it("keeps the real invoice method for a manager-PIN 'manual' override (scoping lock)", async () => {
    const t = convexTest(schema);
    const { staffId, sessionId, outletId } = await seedStaff(t, "manager");
    const txnId = await seedPaidTxn(t, staffId, outletId, Date.now(), {
