 ...
 * shifts lifecycle mutations (signoff / handover-out / lock) to cross the
...
// _managerTakeoverSession_internal deleted (ADR-053): only called from
// _commitManagerTakeover_internal which is also deleted. The managerOverride
// action (shiftsInternal.ts) handles the force-logout path without creating a
// new session — the displaced or original staffer re-authenticates normally.
