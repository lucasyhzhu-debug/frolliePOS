...
> **Codegen prerequisite:** Vitest imports symbols from `convex/_generated/api`. After editing `convex/catalog/internal.ts`, ensure codegen has refreshed those generated types — either `npx convex dev` is running in another shell (auto-regen on file save), or run `npx convex codegen` once before vitest. Otherwise `internal.catalog.internal._createInventorySkuCommit_internal` will be `undefined` and the test will fail with a misleading TypeScript / runtime error.
...
undefined. Audit verb inventory_sku.created, source booth_inline, metadata
includes sku/name/low_threshold for forensics. No pos_stock_levels seed —
upsertStockLevel lazy-inits on first movement."
```
...
Append to `convex/catalog/__tests__/productAdmin.test.ts`. The file already imports `{ api, internal }` at line 4 — reuse those; do NOT add a second `internal` import alias.
...
## Task 11: Documentation — SCHEMA, API_REFERENCE, ADR-045, CHANGELOG, CLAUDE.md
...
- [ ] **Step 5: Update CLAUDE.md rule #22's manager-PIN list**

In `CLAUDE.md`, locate rule #22 — search for `**v0.5.3b adds:**` or the surrounding sentence "Manager-admin writes are tiered". The current list reads:

> **manager-PIN** for identity/money (`createStaff`, `setStaffRole`, `deactivateStaff`, `createProduct`, `updateProductPricing`)

Append `createInventorySku` to that parenthesised list so it reads:

> **manager-PIN** for identity/money (`createStaff`, `setStaffRole`, `deactivateStaff`, `createProduct`, `updateProductPricing`, `createInventorySku`)

(One inline word at the end of the list — no other CLAUDE.md changes needed in this phase.)

- [ ] **Step 6: Commit**

```bash
git add docs/SCHEMA.md docs/API_REFERENCE.md docs/ADR/045-route-chunk-reload-boundary.md docs/CHANGELOG.md CLAUDE.md
git commit -m "docs(v0.5.5): SCHEMA audit verb + API_REFERENCE + ADR-045 + CHANGELOG + CLAUDE.md
...
- CLAUDE.md: append createInventorySku to rule #22 manager-PIN list"
```
...
> **Codegen prerequisite:** before running vitest, ensure `convex/_generated/api.d.ts` reflects the new `catalog.createInventorySku` action and the extended `catalog.createProduct` signature. If `npx convex dev` has been running throughout, codegen has refreshed automatically on each save. Otherwise run `npx convex codegen` once.
