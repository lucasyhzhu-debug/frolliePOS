...
| a | `approvals.actions.approveShiftOverride` | `{ token: string, managerStaffCode: string, managerPin: string, resultingState: "close" \| "release", idempotencyKey }` | `{ resolved: true }` | Action (Node) *(v1.3.1)*. Off-booth `/approve/:token` PIN-submit endpoint for `kind: "shift_override"`. Token-auth before cache lookup (ADR-046 + ADR-029). argon2 manager PIN verify; on wrong-PIN the per-token `failed_pin_attempts` increments (auto-deny at cap 5) and a `countTowardLockout: false` failed-attempt is audited — it does **NOT** write a booth `pos_auth_attempts` row (SEC-07 isolation: an off-booth attempt cannot DoS-lock a booth login — same isolation as `approveRefund`/`approveSpoilage`). On success: delegates to `shifts/internal._managerOverrideCommit_internal` with `expectedShiftId: context.shift_id` (stale-snapshot guard) + `closeOutlet: resultingState === "close"` + `source: "telegram_approval"`, then `_markResolved_internal`. Audits `shift_override.approval_resolved` via `KIND_AUDIT`. Errors: `SHIFT_CHANGED` (the live booth holder no longer matches the request's snapshot — handover/turnover/natural end during the token TTL; nothing is written), plus the standard `WRONG_KIND` / `TOKEN_EXPIRED` / `REQUEST_RESOLVED` / `NOT_MANAGER` / `INVALID_PIN`. |
...
| Type | Name | Args | Returns | Notes |
|---|---|---|---|---|
| m | `shifts.shifts.openBooth` | `{ idempotencyKey, sessionId, steps, openCount? }` | `{ ok: true; shiftId }` | Start-of-day SOP. **Requires outlet closed** (`BOOTH_ALREADY_OPEN`). Sets `outlets.is_open = true` (Level 1) AND inserts a `pos_shifts` holder row (`started_via: "sop"`, Level 2). Audits `outlet.opened`. |
| m | `shifts.shifts.startShift` | `{ idempotencyKey, sessionId, steps, openCount? }` | `{ ok: true; shiftId }` | Incoming staff on an **already-open, holderless** outlet (post-handover). Rejects if closed (`BOOTH_NOT_OPEN`) or a holder exists (`SHIFT_IN_PROGRESS`). Inserts a `pos_shifts` row (`started_via: "handover"`, `prev_shift_id` linked). Level 2 only. Audits `shift.start`. |
| m | `shifts.shifts.endOfDay` | `{ idempotencyKey, sessionId, steps, closeCount? }` | `{ ok: true; durationMs }` | Ends the active `pos_shifts` row AND sets `outlets.is_open = false`. **Idempotent**: on an already-closed outlet it just ends the session and returns `durationMs: 0` (no duplicate close/summary). Audits `outlet.closed`. Schedules `_sendSignoffSummary` → per-outlet managers Telegram. |
| m | `shifts.shifts.handover` | `{ idempotencyKey, sessionId, steps, closeCount? }` | `{ ok: true; durationMs }` | Outgoing holder ends their `pos_shifts` row; the **outlet stays open** (Level 1 untouched) so the incoming staff can `startShift`. Ends the session; schedules `_sendSignoffSummary`. Guards `NO_ACTIVE_SHIFT` / `NOT_SHIFT_HOLDER`. No `handover_pending` state. Audits `shift.handover`. |
| m | `shifts.shifts.lock` | `{ idempotencyKey, sessionId }` | `{ ok: true }` | Ends the session (`manual_lock`). The `pos_shifts` holder row and `outlets.is_open` are **unchanged** — the locked staff re-authenticates via standard login to resume (no separate resume mutation; the holder was never released). Audits `shift.lock`. |
| q | `shifts.shifts.loginContext` | `{ deviceId: string }` | `{ outletOpen: boolean; holderStaffId: Id<"staff"> \| null; holderName: string \| null }` | Session-less reactive query for the login gate. The FE derives: closed → SOP; open + no holder → incoming (`startShift`); open + holder is me → resume; open + holder is someone else → block + manager override. |

### Actions (Node runtime — `convex/shifts/actions.ts`)

| Type | Name | Args | Returns | Notes |
|---|---|---|---|---|
| a | `shifts.actions.managerOverride` | `{ idempotencyKey, deviceId, managerStaffId: Id<"staff">, managerPin: string, resultingState: "close" \| "release" }` | `{ ok: true }` | **Manager-PIN gated** (argon2id, ADR-046). Booth-inline override path (present manager picked from the booth list → `managerStaffId`). `resultingState: "release"` ends the active hold only; `"close"` also clears `outlets.is_open = false` (valid even if there is no active hold). Passes NO `expectedShiftId` to the commit — the manager is present, acting on live state, so the off-booth stale-shift guard does not apply. No new session created; the original staffer re-authenticates via standard login. Errors: `NOT_MANAGER`, `INVALID_PIN`, `LOCKED_OUT:<secs>`. Audits `shift.manager_override` with `metadata.resulting_state` + `source: "booth_inline"`. |
...
| `_managerOverrideCommit_internal` | Atomic commit for both booth-inline `managerOverride` and off-booth `approveShiftOverride`. Args: `closeOutlet: boolean`, `source: "booth_inline" \| "telegram_approval"`, `expectedShiftId?: string` (off-booth only). Pipeline: resolve outlet from device → read active hold → **if `expectedShiftId` is provided AND the live holder's `_id` ≠ `expectedShiftId` (or there is no holder) → throw `SHIFT_CHANGED` before any write** (off-booth stale-snapshot guard; the inline path omits `expectedShiftId` and is unaffected); if `closeOutlet` → set `outlets.is_open = false` **and audit `outlet.closed` with `metadata.via: "manager_override"`** (so the Level-1 close is traced even on the no-holder path); if active hold exists → set `ended_at` + `end_reason: "manager_override"` + audit `shift.manager_override` with `metadata.resulting_state` + `source`. Runs even with no active hold (the close-outlet path is valid without a holder — for the inline override). *(v1.3.1 — previously single-path for booth-inline only)* |
...
| Type | Name | Args | Returns | Notes |
|---|---|---|---|---|
| q | `cockpit.outlets.listOutlets` | `{ sessionId }` | `{ _id, name, code, address?, timezone, active, created_at }[]` | Cockpit-session gated. All active outlets, projected (no sensitive fields). |
...
| a | `cockpit.outlets.createOutlet` | `{ idempotencyKey, sessionId, mode ("blank"\|"clone"), source_outlet_id?, name, code, address?, geo?, timezone, settings, staff_ids, provision_managers_chat }` | `{ outlet_id: Id<"outlets"> }` | Cockpit-session gated. Idempotent via `withActionCache` (auth-before-lookup, ADR-046) + inner `_createOutletAtomic_internal` (`:commit` key). `mode:"clone"` copies active catalog rows (`pos_products`, `pos_inventory_skus`, `pos_product_components`, `pos_settings`; photo `storage_id` reused without copy) — stock NOT cloned. `mode:"blank"` seeds an empty `pos_settings` row. Both paths grant `staff_outlet_access` for the supplied `staff_ids`. Errors: `OUTLET_CODE_TAKEN`, `SOURCE_OUTLET_REQUIRED` (clone path only). Audits `outlet.created` (`source: "cockpit"`). |
| i | `cockpit.outlets._createOutletAtomic_internal` *(internalMutation)* | `{ idempotencyKey, ownerStaffId, mode, source_outlet_id?, name, code, address?, geo?, timezone, settings, staff_ids, provision_managers_chat }` | `{ outlet_id: Id<"outlets"> }` | Single writer. `withIdempotency`-wrapped (`cockpit._createOutletAtomic_internal`). Creates the `outlets` row via `insertOutletRow`, then clones (via `cloneCatalogRows` + `cloneSettingsRow`) or seeds blank (`seedSettingsRow`), then calls `grantOutletAccessRow`. All within one Convex transaction — any throw rolls back all writes. |
