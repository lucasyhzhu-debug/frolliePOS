...
- `convex/receipts/` *(v0.5.1 PR A)* — receipt rendering: `pos_receipt_html_cache` cache table, public `/r/<token>` httpAction, hardcoded template (ADR-039 §4). `_renderReceiptByToken_internal`, `_buildViewModel_internal`, `_getCachedReceipt_internal`, `_writeCacheEntry_internal`, `_purgeReceiptCache_internal` (stub in PR A — real assertion-throw in PR B), `_lazyMintReceiptToken_internal` (dormant in v0.5.1; v0.5.3 history view caller will invoke). `handleReceiptRoute` is the `GET /r/:token` httpAction (24h cache, 404 page in Indonesian).
