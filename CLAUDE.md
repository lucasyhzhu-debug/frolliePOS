...
- `convex/telegram/` *(v0.4 rewrite)* — production Telegram integration. Key files:
  - `chatRegistry.ts` — self-registration registry: `telegramChats` table management; `getChatIdByRole` (role-based routing with `TELEGRAM_CHAT_ID` env-fallback); `mgrListChats` / `mgrAssignRole` / `mgrArchiveChat` / `mgrRestoreChat` / `mgrSendTest` (manager-session-gated public surface at `api.telegram.chatRegistry.mgr*`).
